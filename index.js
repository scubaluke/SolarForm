...
const theAFID = document.querySelector('#AFID');
theAFID.value = window.location.href.split('AFID=')[1] || '465368';
console.log(theAFID);

// TCPA CONSENT
const agreeInput = document.querySelector('#TCPA');
agreeInput.addEventListener(
  'click',
  () => (document.querySelector('#tcpa_consent').value = 'Yes')
);
...
const phoneInput = document.querySelector('#phone_primary');
...
async function validateAndSubmit(e) {
  ...
  if (!form.phone_primary.value) {
    showNotValid(phone_primary, 'Phone Number');
  } else {
    removeRequired(phone_primary);
  ...
  if (
    [...wideInputs].every((input) => input.value) &&
    form.TCPA.checked &&
    isPhoneValid &&
    emailIsValid(form.email_address.value)
  ) {
...
function validateSlide1() {
  ...
  if (
    [...selectInputs].every((input) => input.value) &&
